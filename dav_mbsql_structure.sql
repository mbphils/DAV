...
CREATE DATABASE IF NOT EXISTS mbsql;
use mbsql;
...
A.Avail_Bal,A.DOPEN as "Date Opened",A.Last_trans_date as "Last Transaction" from dav_report A
LEFT JOIN sysparms B on A.br_NaCo = B.BRNAME left join (select * from dav_main_id)C on A.ALTCID = C.CID and C.sysid = A.sysid
where A.DOTRN = Date_
...
CASE when A.APPTYPE = '3' THEN ifnull(fn_DAV_Date_Converter(A.DOPEN),'') END as Issue_Date,
CASE when A.APPTYPE = '3' THEN ifnull(fn_DAV_Date_Converter(A.MATDATE),'') END as Mat_date,
...
when X.TRNCHG > 0 then fn_DAV_Date_Converter(X.TRNDATE) end,'') end as "Dorm_ch_date",fn_DAV_Date_Converter(A.DOLASTTRN) as "Last_trans_date",
...
BEGIN
	...
	ifnull(A.`Issue Date`,'')as `Issue Date`,ifnull(A.Mat_date,'')as Mat_date,
	ifnull(DATEDIFF(STR_TO_DATE(A.Mat_date,'%m/%d/%YY'),
	STR_TO_DATE(A.`Issue Date`,'%m/%d/%YY')),'')as Term,A.Avail_Bal from dav_report A
	LEFT JOIN (select SYSTEMID,BRNAME,ORGNAME,DOLASTMLY,DOLASTQLY from sysparms)B 
	on A.Br_NaCo = B.BRNAME
	where A.DOTRN = Date_
